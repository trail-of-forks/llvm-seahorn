...
Instruction *SeaInstCombinerImpl::foldAddLikeCommutative(Value *LHS, Value *RHS,
                                                      bool NSW, bool NUW) {
  Value *A, *B, *C;
...
Instruction *SeaInstCombinerImpl::visitAdd(BinaryOperator &I) {
  ...
  if (!AvoidBv &&
      haveNoCommonBitsSet(LHSCache, RHSCache, SQ.getWithInstruction(&I)))
...
Instruction *SeaInstCombinerImpl::visitSub(BinaryOperator &I) {
  ...
  const APInt *Op0C;
  if (!AvoidBv && match(Op0, m_APInt(Op0C))) {
    if (Op0C->isMask()) {
...
Instruction *
SeaInstCombinerImpl::hoistFNegAboveFMulFDiv(Value *FNegOp,
