...
static cl::opt<bool> EnableCodeSinking("sea-instcombine-code-sinking",
...
static cl::opt<unsigned> MaxArraySize(
    "sea-instcombine-maxarray-size", cl::init(1024),
...
Value *SeaInstCombinerImpl::EmitGEPOffset(GEPOperator *GEP, bool RewriteGEP) {
  if (!RewriteGEP)
...
//        -> (cmp pred C', (ctpop x))
Instruction *SeaInstCombinerImpl::tryFoldInstWithCtpopWithNot(Instruction *I) {
...
Instruction *SeaInstCombinerImpl::foldBinOpShiftWithShift(BinaryOperator &I) {
...
Instruction *SeaInstCombinerImpl::foldBinOpOfSelectAndCastOfSelectCondition(
    BinaryOperator &I) {
...
SeaInstCombinerImpl::matchSymmetricPair(Value *LHS, Value *RHS) {
...
Instruction *SeaInstCombinerImpl::foldFBinOpOfIntCastsFromSign(
...
//    2) (fp_binop ({s|u}itofp x), FpC)
//        -> ({s|u}itofp (int_binop x, (fpto{s|u}i FpC)))
Instruction *SeaInstCombinerImpl::foldFBinOpOfIntCasts(BinaryOperator &BO) {
...
Instruction *SeaInstCombinerImpl::FoldOpIntoSelect(Instruction &Op,
                                                   SelectInst *SI,
...
Instruction *
SeaInstCombinerImpl::visitGetElementPtrInst(GetElementPtrInst &GEP) {
  Value *PtrOp = GEP.getOperand(0);
...
bool SeaInstCombinerImpl::removeInstructionsBeforeUnreachable(Instruction &I) {
...
void SeaInstCombinerImpl::addDeadEdge(BasicBlock *From, BasicBlock *To,
...
void SeaInstCombinerImpl::handleUnreachableFrom(
...
void SeaInstCombinerImpl::handlePotentiallyDeadBlocks(
    SmallVectorImpl<BasicBlock *> &Worklist) {
  while (!Worklist.empty()) {
...
void SeaInstCombinerImpl::handlePotentiallyDeadSuccessors(
    BasicBlock *BB, BasicBlock *LiveSucc) {
...
/// block.
bool SeaInstCombinerImpl::tryToSinkInstruction(Instruction *I,
...
void SeaInstCombinerImpl::tryToSinkInstructionDbgValues(
...
void SeaInstCombinerImpl::tryToSinkInstructionDbgVariableRecords(
    Instruction *I, BasicBlock::iterator InsertPos, BasicBlock *SrcBlock,
    BasicBlock *DestBlock,
...
bool SeaInstCombinerImpl::prepareWorklist(Function &F) {
...
static bool combineInstructionsOverFunction(
    ...
    BranchProbabilityInfo *BPI, ProfileSummaryInfo *PSI, bool AvoidBv,
    bool AvoidUnsignedICmp, bool AvoidIntToPtr, bool AvoidAliasing,
    bool AvoidDisequalities, const SeaInstCombineOptions &Opts) {
...
SeaInstCombinePass::SeaInstCombinePass(bool AvoidBv, bool AvoidUnsignedICmp,
                                       bool AvoidIntToPtr, bool AvoidAliasing,
    ...
    : AvoidBv(AvoidBv), AvoidUnsignedICmp(AvoidUnsignedICmp),
      ...
      AvoidDisequalities(AvoidDisequalities), Options(Opts) {}

void SeaInstCombinePass::printPipeline(
    raw_ostream &OS, function_ref<StringRef(StringRef)> MapClassName2PassName) {
  static_cast<PassInfoMixin<SeaInstCombinePass> *>(this)->printPipeline(
...
PreservedAnalyses SeaInstCombinePass::run(Function &F,
  ...
  if (!combineInstructionsOverFunction(
          F, Worklist, AA, AC, TLI, TTI, DT, ORE, BFI, BPI, PSI, AvoidBv,
          AvoidUnsignedICmp, AvoidIntToPtr, AvoidAliasing, AvoidDisequalities,
          Options)) {
...
SeaInstructionCombiningPass::SeaInstructionCombiningPass(
    ...
    : FunctionPass(ID), AvoidBv(AvoidBv), AvoidUnsignedICmp(AvoidUnsignedICmp),
      AvoidIntToPtr(AvoidIntToPtr), AvoidAliasing(AvoidAliasing),
...
FunctionPass *createSeaInstructionCombiningPass() {
  ...
  return new SeaInstructionCombiningPass(AvoidBv, AvoidUnsignedICmp,
                                         AvoidIntToPtr, AvoidAliasing,
                                         AvoidDisequalities);
}

FunctionPass *createSeaInstructionCombiningPass(bool AvoidBv,
