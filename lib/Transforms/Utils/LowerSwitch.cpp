...
namespace llvm {
void initializeSeaLowerSwitchPass(PassRegistry&);
}

#define DEBUG_TYPE "sea-lower-switch"
...
class SeaLowerSwitch : public FunctionPass {
public:
  static char ID; // Pass identification, replacement for typeid
  SeaLowerSwitch() : FunctionPass(ID) {
    initializeSeaLowerSwitchPass(*PassRegistry::getPassRegistry());
  }
...
struct CaseCmp {
  bool operator()(const SeaLowerSwitch::CaseRange &C1,
                  const SeaLowerSwitch::CaseRange &C2) {
...
char SeaLowerSwitch::ID = 0;
INITIALIZE_PASS(SeaLowerSwitch, "sealowerswitch", "Lower SwitchInst's to branches",
                false, false)

// Publicly exposed interface to pass...
// char &llvm::LowerSwitchID = LowerSwitch::ID;
// createLowerSwitchPass - Interface to this file...
FunctionPass *llvm_seahorn::createLowerSwitchPass() {
  return new SeaLowerSwitch();
}

bool SeaLowerSwitch::runOnFunction(Function &F) {
  bool Changed = false;
...
static raw_ostream &operator<<(raw_ostream &O, const SeaLowerSwitch::CaseVector &C)
    LLVM_ATTRIBUTE_USED;
static raw_ostream &operator<<(raw_ostream &O,
                               const SeaLowerSwitch::CaseVector &C) {
  O << "[";

  for (SeaLowerSwitch::CaseVector::const_iterator B = C.begin(), E = C.end();
       B != E;) {
    O << *B->Low << " -" << *B->High;
    if (++B != E)
      O << ", ";
...
/// stack.
BasicBlock *
SeaLowerSwitch::switchConvert(CaseItr Begin, CaseItr End, ConstantInt *LowerBound,
                           ConstantInt *UpperBound, Value *Val,
                           BasicBlock *Predecessor, BasicBlock *OrigBlock,
                           BasicBlock *Default,
                           const std::vector<IntRange> &UnreachableRanges) {
...
BasicBlock *SeaLowerSwitch::newLeafBlock(CaseRange &Leaf, Value *Val,
...
unsigned SeaLowerSwitch::Clusterify(CaseVector &Cases, SwitchInst *SI) {
  unsigned numCmps = 0;
...
void SeaLowerSwitch::processSwitchInst(SwitchInst *SI,
                                    SmallPtrSetImpl<BasicBlock *> &DeleteList) {
